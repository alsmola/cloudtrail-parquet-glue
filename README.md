...
# To use
- Define the module and supply the information from your CloudTrail setup. You should create three additional S3 buckets - one used for Glue ETL scripts (`etl_script_s3_bucket`), one used for temporary processing (`temp_s3_bucket`), and one used to store the parquet files (`parquet_s3_bucket`).
...
module "cloudtrail_parquet_glue" {
  ...
  cloudtrail_s3_bucket = "s3://my-cloudtrail-bycket"
